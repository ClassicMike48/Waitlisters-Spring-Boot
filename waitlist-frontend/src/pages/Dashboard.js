...
function Dashboard({ setToken, useAuth }) {
  ...
  const [editing_instructor_join_code, setEditingInstructorJoinCode] =
    useState(false);

  // state variables for the resulting join codes after creating a course.
  const [display_join_codes_after_create, setDisplayJoinCodesAfterCreate] =
    useState(false);
  const [output_instructor_join_code, setOutputInstructorJoinCode] =
    useState("");
  ...
  async function getCoursesEnrolledAsStudent() {
    ...
    if (!response.ok) {
      console.error("Error loading student courses: ", response.statusText);
      return;
    }
    const dataOut = await response.json();
  ...
  async function getJoinCodes(new_course_id) {
    ...
    if (!response.ok)
      throw new Error(`Error loading join codes: ${response.statusText}`);
    const dataOut = await response.json();
          ...
          <div className="card text-center">
            ...
            <div className="card-body">
              <div className="tab-content">
                {/* instructor tab*/}
                <div className="tab-pane fade show active" id="instructor">
                              ...
                              {display_join_codes_after_create ? (
                                <>
                                  <p>
                                    ...
                                    Student Join code:{" "}
                                    {output_student_join_code} <br></br>
                                    Instructor Join code:{" "}
                                    {output_instructor_join_code} <br></br>
                  ...
                  <div className="row py-2 justify-content-center">
                    <div className="col-sm-6">
                      <button
                        type="button"
                        className="btn btn-dark"
                        onClick={handleEditingInstructorJoinCode}
                      >
                      ...
                      {editing_instructor_join_code ? (
                        <>
                          <div>
                            <form
                              className="form-group"
                              onSubmit={handleSubmitInstructorJoinCourse}
                            >
                              <label
                                for="join_code"
                                class="form-label mt-3 mb-1"
                              ...
                              <input
                                ...
                                className="form-control mb-1"
                                type="text"
                                value={course_instructor_join_code}
                  ...
                  <div className="row py-2 justify-content-center">
                    <div className="col-sm-6">
                      <button
                        ...
                        onClick={handleEditingStudentJoinCode}
                      >
                    ...
                    <div className="col-sm-6">
                      ...
                      <div className="dropdown">
                        ...
                        <ul className="dropdown-menu">
                          {student_course_list.map((student_course_list_i) => (
                            <li className="dropdown-item justify-content-center text-center">
                              <h2 key={"SC" + student_course_list_i.course_id}>
                                <Link
